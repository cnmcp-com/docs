...
在本教程中,您将学习如何构建一个基于 LLM 的聊天机器人客户端,该客户端可以连接到 MCP 服务器。建议您先完成[服务器快速入门](/quickstart/server)教程,该教程将指导您完成构建第一个服务器的基本知识。
...
如果您连接到服务器快速入门中的天气服务器,它应该看起来像这样:
...
本示例演示如何构建一个交互式聊天机器人,将 Spring AI 的模型上下文协议(MCP)与 [Brave Search MCP 服务器](https://github.com/modelcontextprotocol/servers/tree/main/src/brave-search)结合使用。该应用程序创建一个由 Anthropic 的 Claude AI 模型驱动的对话界面,可以通过 Brave Search 执行互联网搜索,实现与实时网络数据的自然语言交互。
...
应用程序将启动一个交互式聊天会话,您可以在其中提问。当聊天机器人需要从互联网查找信息来回答您的查询时,它将使用 Brave Search。
...
- 具有多个传输类型的多个客户端: `STDIO` 和 `SSE`(服务器发送事件)
