...
采样是 MCP 的一个强大功能，允许服务器通过客户端请求 LLM 补全，实现复杂的代理行为，同时保持安全性和隐私性。
...
这种人工参与的设计确保用户能够控制 LLM 看到和生成的内容。
...
```typescript
{
  ...
  modelPreferences?: {
    ...
    intelligencePriority?: number  // 0-1，高级模型能力的重要性
  },
  systemPrompt?: string,
...
- `hints`：模型名称建议数组，客户端可以使用它来选择适当的模型：
...
可选的 `systemPrompt` 字段允许服务器请求特定的系统提示。客户端可以修改或忽略它。

### 上下文包含

`includeContext` 参数指定要包含什么 MCP 上下文：
