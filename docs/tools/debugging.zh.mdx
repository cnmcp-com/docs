...
1. **MCP 检查器**
   - 交互式调试界面
   - 直接服务器测试
   - 查看[检查器指南](/docs/tools/inspector)了解详情
...
- 通过 `claude_desktop_config.json` 启动的服务器的工作目录可能是未定义的（如 macOS 上的 `/`），因为 Claude Desktop 可以从任何位置启动
...
要覆盖默认变量或提供您自己的变量，您可以在 `claude_desktop_config.json` 中指定 `env` 键：
...
当构建使用本地 stdio [传输](/docs/concepts/transports)的服务器时，所有记录到 stderr（标准错误）的消息都将被主机应用程序（如 Claude Desktop）自动捕获。
